...
3. **Wrap-only LAMMPS data convention** everywhere — core topon, simbox, *and* topro. Atom positions are written inside `[0, box)`; the data file has 7-column atom rows (no `ix iy iz` image flags). LAMMPS handles bonds across the periodic boundary through the neighbor / ghost-atom system, which is correct as long as every bond is shorter than `box/2` (always the case for our KG / DREIDING / MARTINI 3 setups — e.g. MARTINI's ~3.6 Å bonds in a ≥240 Å box). An image-flag attempt in early protein-network versions produced unphysical box-spanning bonds; see DEVELOPMENT.md. *LAMMPS "inconsistent image flags" warnings are expected with wrap-only data and are not bugs.*
