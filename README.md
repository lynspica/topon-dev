...
<p align="center">
  <sub>The network is generated by topon's own sculpting generator, the letters are a colouring of the strands, the pinches inside them are real entanglement kinks, and the motion is a Kremer–Grest minimisation and equilibration run in LAMMPS. <a href="assets/logo/">How it was made →</a></sub>
</p>
...
strands are, what chemistry sits on them — and topon turns that into a simulation
box. Connectivity comes first; chemistry is mapped onto it afterwards. That split
is the whole idea, and it means the same graph can come out as a coarse-grained
melt or as an all-atom system.
...
topon starts from a lattice of junctions, then **prunes it**. A bare lattice gives
every junction the same functionality, which no real network has. The sculpting
generator removes edges until the degree distribution matches what you asked for:
...
<sub>Left: nodes coloured by current degree — deep blue is 6-connected, warming as
they lose edges. Right: the degree histogram filling in against the dashed target.
This is the real algorithm, replayed from the edge-removal history it recorded:
228 removals taking a 6×6×6 lattice from 648 edges to 420, mean degree 6.00 → 3.89.</sub>
...
degree 4.0 — a tetrafunctional network, whichever lattice you begin with.</sub>
...
junctions, so their bonds start far too short — about 5× in the coarse-grained
case, 2.5× all-atom. **MD is what makes it real:** the chains coil out to their
natural length and the lattice becomes a melt, carrying the connectivity you asked
for with it.
...
<tr>
<td><b>Coarse-grained</b> — Kremer–Grest, 5 125 beads, 250 strands. Bonds go
0.17&nbsp;σ → 0.97 as the lattice coils into a melt.</td>
<td><b>All-atom</b> — the same construction in DREIDING PDMS, 10 896 atoms. Gold
silicon and red oxygen trace the siloxane backbone; bonds go 0.44&nbsp;Å → 1.12.</td>
</tr>
</table>
...
<sub>Left: a block copolymer — every strand half A (red), half B (blue), and the
pattern survives the melt because sequence is a property of the chain, not the
...
strands off their lattice edges so they hook through each other. Here is one of
them, in the network and up close, relaxing:
...
<sub>The two entangled chains are gold and violet; everything else is grey. On the
lattice they are a tight hooked crossing (closest approach ≈ 0.4&nbsp;σ); as the
network melts they open into interlocked coils, still hooked.</sub>

You can also inject loop defects — parallel edges between junctions that are
already connected — with valence protection so it never over-coordinates a
junction. POSS chain caps come with real Si₈O₁₂ cage chemistry (caps only: POSS
at an internal junction is a known bug, and `topon doctor` will tell you so).

---

## Protein networks

The protein side — **topro**, implemented in `topon.protein_network` — takes a
residue sequence and a repeat count and builds a crosslinked protein network,
solvated, ready to run. Both MARTINI 3 and CHARMM36m all-atom.
...
form (red) and the clusters merge; the teal giant cluster grows until it spans
everything. The curve tracks the largest cluster against conversion; the marked
line is where the <i>last</i> chain joins, at conversion 0.55. The giant component
passes half the chains much earlier, around 0.14.</sub>
...
backbone from the force field's own internal-coordinate tables — correct
L-chirality, trans omega with a tunable X-Pro *cis* fraction, and restraints that
hold both through minimisation. It reads a topology written by the step above, so
it is two commands:
...
(`--physical-backbone` is off by default, which gives the older jittered
placement — see [`examples/demos/protein/charmm/`](examples/demos/protein/charmm/)
for a scripted version of both steps.)
